...
factors — especially from your web search results. Cite concrete findings \
(polls, schedules, precedent, expert consensus). Each factor should push the \
probability in a stated direction by a stated amount.
...
merely "very likely."
5. **Weigh recency.** Prefer information from web search results over your \
training data when they conflict. Note if search results are sparse or \
inconclusive.
6. **State uncertainty honestly.** If you have little relevant knowledge even \
after searching, say so. A wider confidence band (lower confidence) is better \
than false precision.

OUTPUT FORMAT — after completing your searches, respond with **only** a JSON \
...
- reasoning: show your work — state the base rate, key evidence from search \
results, your updates, and the overconfidence adjustment you applied.
- key_factors: 3-6 concrete factors that most influenced your estimate, \
preferring facts found via web search."""
...
Search for recent news and information about this question, then estimate \
the probability that it resolves YES."""
...
def _extract_final_text(content_blocks: list) -> str:
    """Extract the final text block from a response that may contain tool use blocks."""
...
def _count_search_queries(content_blocks: list) -> int:
    """Count how many web_search tool_use blocks are in the response."""
    count = 0
...
def _extract_search_sources(content_blocks: list) -> List[str]:
    """Extract URLs from web search result blocks for logging."""
    sources: List[str] = []
    for block in content_blocks:
        if getattr(block, "type", None) == "web_search_tool_result":
            for result in getattr(block, "content", []):
                ...
                if url:
                    sources.append(url)
    return sources[:10]  # Cap at 10 for logging
...
async def estimate_probability(
    ...
    """Call Claude with web search to estimate the probability of a market question.

    Claude first searches for recent news, then produces a calibrated estimate.
    ...
    logger.info("Requesting estimate with web search from %s for: %s", MODEL, question[:80])
    agent_log("analysis", "estimator", f"Web search + estimate for: {question[:80]}")
    ...
    # Log search activity
    search_count = _count_search_queries(message.content)
    sources = _extract_search_sources(message.content)
    if search_count > 0:
        agent_log(
            "analysis", "estimator",
            f"Web search completed: {search_count} queries, {len(sources)} sources",
            {"market_id": market_id, "sources": sources[:5]},
        )
    else:
        agent_log("info", "estimator", "No web searches performed", {"market_id": market_id})

    raw_text = _extract_final_text(message.content)
    logger.debug("Raw Claude response: %s", raw_text[:500])

    if not raw_text:
        logger.error("No text block in Claude response for market %s", market_id)
    ...
    logger.info(
        "Estimate for %s: prob=%.3f confidence=%s (searches=%d)",
        market_id,
