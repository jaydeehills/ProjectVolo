...
const LEVEL_STYLES: Record<string, { color: string; bg: string }> = {
  ...
  analysis: { color: "text-purple-400", bg: "bg-purple-500/10" },
  signal: { color: "text-emerald-400", bg: "bg-emerald-500/10" },
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg flex flex-col h-full">
      ...
      <div className="px-3 py-3 border-b border-zinc-800 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-zinc-100">Agent Log</h2>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-2 space-y-0.5 font-mono text-[11px] leading-relaxed">
        {loading && logs.length === 0 ? (
          <p className="text-zinc-700 p-2 animate-pulse">Connecting...</p>
        ) : logs.length === 0 ? (
          <p className="text-zinc-700 p-2">
          ...
          logs.map((entry, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-zinc-500 shrink-0 w-[56px] truncate">
                  {entry.module}
                </span>
